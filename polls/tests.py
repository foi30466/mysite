...
class QuestionModelTests(TestCase):
	...
	def test_was_published_recently_old_questions(self):
	   """was_published_recently() returns False for questions
		   whose pub_date is older than 2 days"""
	   time = timezone.now() + datetime.timedelta(days=2, seconds=1)
	   old_question = Question(pub_date=time)
	   self.assertIs(old_question.was_published_recently(), False)
		
	def test_was_published_recently_recent_questions(self):
	   """was_published_recently() returns True for questions
		   whose pub_date is within the last 2 days"""
...
def create_question(question_text, days):
	"""Create a question with 'question_text' and published with 'days' offset to now
	(negative for past, positive for future questions)"""
